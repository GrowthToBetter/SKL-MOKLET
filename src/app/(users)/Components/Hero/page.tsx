...
export default function Hero(props: any) {
  ...
  const [teacherData, setTeacherData] = useState<teacherFullPayload | null>(
    null
  );
      ...
      if (session) {
        try {
          const response = await fetch(
            `/api/teacher?teacherId=${props.userData?.teacherId}`,
            {
              method: "GET",
            }
          );
          if (response.ok) {
            const teacher: teacherFullPayload = await response.json();
        ...
        <>
          ...
          >
            ...
            <div className="rounded-full overflow-hidden -mt-12 relative w-[60px] h-[60px] ml-4">
              <Image
                src={teacherData.photo_profile as string}
                height={60}
                width={60}
                alt="image"
                className="absolute"
          ...
          <div>
            ...
            <p>
              ...
              rerum impedit, nesciunt laboriosam sint non. Voluptatem laboriosam
              omnis atque, cum inventore alias fuga necessitatibus? Esse quo
              atque dolorum accusamus. Aliquam.
            </p>
          </div>
        </>
      ) : (
